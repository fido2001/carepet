                    ...
                    <tbody>
                        ...
                        {{-- @foreach ($dataPaket as $no => $paket) --}}
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('historyDetail.paket.admin', $pemesanan->id) }}" class="badge badge-info">Detail</a>
                                    @if ($pemesanan->bukti_pembayaran == null)
                                    <a href="#" data-id="{{ $pemesanan->id }}" class="badge badge-danger swal-confirm">
                                        <form action="{{ route('destroy.paket.admin', $pemesanan->id) }}" id="delete{{ $pemesanan->id }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        </form>
                                        Batalkan Pesanan
